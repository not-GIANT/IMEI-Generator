...
def generate_by_pattern(imei1: str, imei2: str, count: int = 10, mode: str = "random") -> list[str] | str:
    ...
    if common_len == 14 and mode == "random":
        return "Both IMEIs are identical. Please provide two different IMEIs to define a pattern."

    start_index = common_len
    var_indices = list(range(start_index, 14))
    
    # Delegate to mask-like logic if we found var indices
    mask_list = list(s1)
...
class IMEIApp(tk.Tk):
    ...
    def _export_results(self, result_box):
    ...
    def _build_validator_tab(self, page):
        ...
        GlowButton(btns, "✕  Clear", command=self._clear_val,
                   color=SUBTEXT, width=110).pack(side="left")
        GlowButton(btns, "💾  Export Valid", command=lambda: self._export_results(self.val_result),
